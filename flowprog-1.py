...
# Page setup
st.set_page_config(layout="wide", page_title="🚗 Vehicle Production Dashboard")
...
st.download_button(
    label="⬇️ Download Vehicle Details as Excel",
...
# ------------------ Section: Update Vehicle Status ------------------
st.subheader("✏️ Update Vehicle Status")

if not df.empty and "VIN" in df.columns:
    update_vin = st.selectbox("Select VIN to Update", df["VIN"])
    ...
    if st.button("Update Status"):
        ...
        if new_status == "Completed":
            next_index = PRODUCTION_LINES.index(update_line) + 1
            if next_index < len(PRODUCTION_LINES):
                df.at[idx, "Current Line"] = PRODUCTION_LINES[next_index]
